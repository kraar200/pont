...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@d8_8q"))
...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@devkimo"))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@v_u_ww"))
    except BaseException:
        pass  
    ...
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython1.send_message(event.chat_id, f"تم الانتهاء من التجميع | kimo")
        ...
        await sython1.send_message(event.chat_id, "تم الانتهاء من التجميع | kimo")
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        await event.reply("جاري تجميع النقاط")
        await event.edit("جاري تجميع النقاط")
        joinu = await sython1(JoinChannelRequest('d8_8q'))
    ...
    while True:
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await sython1(JoinChannelRequest('d8_8q'))
                channel_entity = await sython1.get_entity(pot)
                await sython1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة كيمو**')
                await sython1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await sython1.get_messages(pot, limit=1)
                await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_ids:
        ...
        haso = await sython1.get_entity(chn)
        join = await sython1(JoinChannelRequest(chn))
        joion = await sython1(JoinChannelRequest('d8_8q'))
        somy = await sython1.get_messages(chn, limit=nu)
        await somy[nuu].click(0)
        sleep(1)
...
#code skip accumulate points by t.me/T_5_C thank you my bro
